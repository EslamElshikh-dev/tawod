...
function articleFromDirectory(slug) {
  ...
  const [category, icon] = categoryFor(slug);
  ...
  const image = imageUrl.replace(/^https?:\/\/(?:www\.)?tawodco\.com/i, "") || "/images/blog/construction-building-riyadh.webp";
  ...
  return { slug, title, description, image, datePublished, category, icon };
}
...
function articleCard(article, { featured = false, eager = false } = {}) {
  ...
  return `<article class="article-card${featured ? "" : " reveal"}">
    <div class="card-media">${badge}<a href="${url}" aria-label="${escapeHtml(article.title)}"><img alt="${escapeHtml(article.title)}" decoding="async" loading="${eager ? "eager" : "lazy"}" src="${escapeHtml(article.image)}"></a></div>
    <div class="card-body">
      ...
      <div class="card-tags"><span>${article.category}</span><span>الرياض</span></div>
...
function archiveContent(page, totalPages, articles, featuredArticles) {
  ...
  const heading = page === 1 ? "أحدث المقالات" : `مقالات المدونة — الصفحة ${page}`;
  const intro = page === 1
    ? "أدلة عملية مرتبة بعناية تساعدك على فهم مراحل البناء والتشطيب والترميم، ومقارنة الحلول، واتخاذ قرار أوضح قبل بدء مشروعك في الرياض."
...
function updateHead(html, page, totalPages, pageArticles, articleCount) {
  ...
  const title = page === 1
    ? "مدونة شركة تعاود للمقاولات بالرياض | أحدث مقالات البناء والتشطيب"
    : `مقالات المقاولات والبناء — الصفحة ${page} | شركة تعاود بالرياض`;
  const description = page === 1
    ? `اكتشف ${articleCount} مقالًا عن المقاولات والبناء والتشطيب والترميم في الرياض، مرتبة من الأحدث إلى الأقدم مع 10 مقالات في كل صفحة.`
  ...
  output = replaceHeadTag(output, /<meta\b(?=[^>]*\bproperty=["']og:title["'])[^>]*>/i, `<meta property="og:title" content="${escapeHtml(title)}">`);
  output = replaceHeadTag(output, /<meta\b(?=[^>]*\bproperty=["']og:description["'])[^>]*>/i, `<meta property="og:description" content="${escapeHtml(description)}">`);
